...
void populate(){
	...
	strcpy(table[0].name,"abc");
	table[0].p_n=30178;
	noofdet++;

	strcpy(table[1].name,"def");
	table[1].p_n=30179;
	noofdet++;
...
int main(){
        ...
        struct sockaddr_in orgtns;
        struct sockaddr_storage lns;

        orgtns.sin_family=AF_INET;
        orgtns.sin_port=htons(30197);
       	orgtns.sin_addr.s_addr=inet_addr("127.0.0.1");

        if(sid==-1){
                printf("\n\n\n\nError in Socket creation");
        }

        bd=bind(sid,(struct sockaddr*)&orgtns,sizeof(struct sockaddr));
        if(bd==-1){
                printf("\n\n\n\nbind failed..");
        }
